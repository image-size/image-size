...
function extractTags (ifdBuffer, isBigEndian) {
  ...
  while (ifdBuffer.length) {
    ...
    type = ifdBuffer.readUInt(16, 2, isBigEndian);
    length = ifdBuffer.readUInt(32, 4, isBigEndian);
    if (code === 0) {
      break;
    } else {
      if (length === 1 && type === 3) {
        var low = ifdBuffer.readUInt(16, 8, isBigEndian);
        var high = ifdBuffer.readUInt(16, 10, isBigEndian);
        tags[code] = (high << 16) + low;
      }
...
function determineEndianness (buffer) {
  ...
  if ('II' === signature) {
    ...
    throw new TypeError('Invalid TIFF, can\'t determine endianness');
  }
}
