...
const calculateRREQLength = (box: DataView, begin: number): number => {
...
const parseIHDR = (box: DataView, offset: number): ISize => {
  return {
...
export const JP2: IImage = {
  ...
  calculate(buffer) {
    ...
    case BoxTypes.jp2h :
      return parseIHDR(buffer, offset + 8)
    default:
      throw new TypeError('Unsupported header found: ' + toAsciiString(buffer, offset, offset + 4))
    }
  }
}
