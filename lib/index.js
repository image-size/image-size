...
var kBufferSize = 4096; /**< MADE: maximum buffer size, with a default of 4096 bytes. */
...
module.exports = function (filepath, callback) {
  ...
  var buffer = new Buffer(kBufferSize);
  ...
  if (typeof callback === 'function') {
    ...
    fs.open(filepath, 'r', function (err, descriptor) {
      ...
      fs.read(descriptor, buffer, 0, kBufferSize, 0, function (err) {
        if (err) {
          return callback(err);
        }
    ...
    fs.readSync(descriptor, buffer, 0, kBufferSize, 0);
